...
public class JediGUI extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float renderPartialTicks) {
		...
		drawString(mc.fontRenderer, "Flora: " + plants, x + 10, py,
				ElementUtils.floraColor);
		py += 10;
		drawString(mc.fontRenderer, "Fauna: " + animals, x + 10, py,
				ElementUtils.faunaColor);
		py += 10;
		drawString(mc.fontRenderer, "Terra: " + earth, x + 10, py,
				ElementUtils.terraColor);
		py += 10;
		drawString(mc.fontRenderer, "Aqua: " + water, x + 10, py,
				ElementUtils.aquaColor);
		py += 20;
		drawString(
				mc.fontRenderer,
				spinner[spinnerIndex].displayName
						+ " level "
						+ String.valueOf(player.inventory.mainInventory[player.inventory.currentItem].stackTagCompound
								.getInteger(spinner[spinnerIndex].internalName) + 1),
				x + 10, py, 0xFFFFFF);
		py += 10;
		drawString(
				mc.fontRenderer,
