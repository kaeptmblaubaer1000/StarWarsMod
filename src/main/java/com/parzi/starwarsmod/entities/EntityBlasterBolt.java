...
public class EntityBlasterBolt extends EntityThrowable {
	...
	public EntityBlasterBolt(World par1World, double par2, double par4,
			double par6) {
	...
	protected void onImpact(MovingObjectPosition pos) {
		if (pos.typeOfHit == MovingObjectType.ENTITY && pos.entityHit != sender) {
			pos.entityHit.attackEntityFrom(
					DamageSource.causePlayerDamage(sender), 5f);
			pos.entityHit.setFire(8);
		} else {
			if (worldObj.getBlock(pos.blockX, pos.blockY + 1, pos.blockZ) == Blocks.air) {
				worldObj.setBlock(pos.blockX, pos.blockY + 1, pos.blockZ,
						Blocks.fire);
			}
		}
		this.setDead();
