...
public class OBJCustomData
{
	...
	{
		//single keys
		FULL_ATLAS("useFullAtlas"), //uv keys
	NORMALIZE_UVS("normalizeUVs"), UNITIZE_UVS("unitizeUVs"), /*WRAP_UVS("wrapUVs"),
		CLAMP_UVS("clampUVs"),
		TILE_UVS("tileUVs"),*/
	FLIP_UVS("flipUVs"), @Deprecated FLIP_V("flip-v"), //group configuration keys
	GROUP_CONFIGURATIONS("groupConfigs"), SHOW("show"), HIDE("hide"), FLAGS("flags"), //group configuration flags
	...
	public void processUVKeys(JsonParser parser, ImmutableMap<String, String> customData)
	{
		for (Keys k : Keys.getProcessUVKeys())
		{
			if (customData.containsKey(k.key))
			{
				...
				if (value.isBoolean())
					this.processUVData.put(k, Pair.of(value.getAsBoolean(), value.getAsBoolean()));
				else if (value.isString())
				{
					if (value.getAsString().equals("u"))
						this.processUVData.put(k, Pair.of(true, false));
					else if (value.getAsString().equals("v"))
						this.processUVData.put(k, Pair.of(false, true));
				}
			}
		}
