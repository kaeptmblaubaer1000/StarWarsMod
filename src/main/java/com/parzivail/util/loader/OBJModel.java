...
public class OBJModel implements IRetexturableModel, IModelCustomData, IModelSimpleProperties
{
	...
	public static class Parser
	{
		...
		public OBJModel parse() throws IOException
		{
			...
			for (; ; )
			{
				...
				{
					if (data.equalsIgnoreCase("obj:white"))
	...
	public static class Face
	{
		...
		public static Vertex[] flipUVs(Vertex[] verts, Pair<Boolean, Boolean> flags)
		{
			...
			for (int i = 0; i < verts.length; i++)
			{
				retVerts[i] = Vertex.copyFrom(verts[i]);
				if (flags.getLeft())
					retVerts[i].texCoord.x = 1 - retVerts[i].texCoord.x;
				if (flags.getRight())
					retVerts[i].texCoord.y = 1 - retVerts[i].texCoord.y;
			}
			return retVerts;
		}
		...
		public Face bake(TRSRTransformation transform, OBJCustomData customData, Map<String, Material> materials)
		{
			...
			if (customData != null && customData.hasProcessed)
			{
				for (Map.Entry<OBJCustomData.Keys, Pair<Boolean, Boolean>> e : customData.processUVData.entrySet())
				{
					...
					{
						case NORMALIZE_UVS:
							vertices = normalizeUVs(vertices, e.getValue(), customData.parsedUVBounds);
							break;
						case UNITIZE_UVS:
							vertices = unitizeUVs(vertices, e.getValue(), this.isTri);
							break;
						case FLIP_UVS:
							vertices = flipUVs(vertices, e.getValue());
							break;
						default:
							break;
					}
	...
	public static class OBJState implements IModelState
	{
		...
		public boolean equals(Object obj)
		{
			...
			return Objects.equal(this.shownConfigs, other.shownConfigs) && Objects.equal(this.parent, other.parent) && Objects.equal(this.materialColorMap, other.materialColorMap) && Objects.equal(this.ignoreHidden, other.ignoreHidden);
		}
	}
	...
	public class OBJBakedModel implements IPerspectiveAwareModel
	{
		...
		public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand)
		{
			...
			if (this.quads == null || !modelState.equals(this.state))
			{
				...
				Optional<TRSRTransformation> transform = modelState.apply(Optional.absent());
				if (modelState instanceof OBJState)
				{
					OBJState s = ((OBJState)modelState);
			...
			for (int e = 0; e < format.getElementCount(); e++)
			{
				switch (format.getElement(e).getUsage())
				{
					case POSITION:
						...
						break;
					case COLOR:
						builder.put(e, v.getMaterial().getColor().x, v.getMaterial().getColor().y, v.getMaterial().getColor().z, v.getMaterial().getColor().w);
						break;
					case UV:
						if (this.model.customData.useFullAtlas)
							builder.put(e, defUV.x, defUV.y, 0, 1);
						else if (sprite.equals(ModelLoader.White.INSTANCE))
							builder.put(e, sprite.getInterpolatedU(defUV.x * 16), sprite.getInterpolatedV(defUV.y * 16), 0, 1);
						else
							builder.put(e, sprite.getInterpolatedU(v.getTextureCoordinate().x * 16), sprite.getInterpolatedV(v.getTextureCoordinate().y * 16), 0, 1);
						break;
					case NORMAL:
