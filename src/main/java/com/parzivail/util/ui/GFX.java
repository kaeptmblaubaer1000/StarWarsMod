...
import static com.parzivail.pswm.PSWM.mc;
...
public class GFX
{
	...
	public static void drawTooltip(int x, int y, String text)
	{
		...
		for (String line : _text)
			if (mc.fontRendererObj.getStringWidth(line) > width)
				width = mc.fontRendererObj.getStringWidth(line);

		int height = (_text.length + 1) * mc.fontRendererObj.FONT_HEIGHT;
		drawBackdrop(x, y, width + 8, height, 255);

		GL11.glEnable(GL11.GL_TEXTURE_2D);
		for (int i = 0; i < _text.length; i++)
			mc.fontRendererObj.drawStringWithShadow(_text[i], x + 6, y + i * (mc.fontRendererObj.FONT_HEIGHT + 1) + 5, GLPalette.WHITE);
	...
	public static void changeCameraDist(int dist)
	{
		try
		{
			ReflectionHelper.setPrivateValue(EntityRenderer.class, mc.entityRenderer, dist, "thirdPersonDistance", "field_78490_B", "E");
			ReflectionHelper.setPrivateValue(EntityRenderer.class, mc.entityRenderer, dist, "thirdPersonDistanceTemp", "field_78491_C", "F");
		}
		catch (Exception e)
		{
	...
	{
		float centerX = event.getResolution().getScaledWidth() / 2f;
		float centerY = event.getResolution().getScaledHeight() / 2f;

		GL11.glPushMatrix();
		PSWM.mc.entityRenderer.setupOverlayRendering();
		...
		Vec3d a = lerp(cursorFrom[0], cursorTo[0], p);
		Vec3d b = lerp(cursorFrom[1], cursorTo[1], p);
		Vec3d c = lerp(cursorFrom[2], cursorTo[2], p);
	...
	public static Vec3d lerp(Point a, Point b, float f)
	{
	...
	public static void renderItem(int x, int y, ItemStack item)
	{
		...
		GL11.glDisable(3008);
		GL11.glDisable(2929);
		RenderItem r = PSWM.mc.getRenderItem();
		r.renderItemIntoGUI(item, x, y);
		r.renderItemOverlayIntoGUI(mc.fontRendererObj, item, x, y, item.getCount() > 0 ? String.valueOf(item.getCount()) : "");
		GL11.glEnable(3008);
		GL11.glEnable(32826);
		GL11.glEnable(2929);
		GL11.glDisable(3042);
