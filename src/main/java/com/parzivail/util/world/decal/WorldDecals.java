package com.parzivail.util.world.decal;
...
public class WorldDecals
{
	...
	{
		Minecraft mc = Minecraft.getMinecraft();
		if (!allDecals.containsKey(dim) || mc == null || mc.renderViewEntity == null || mc.theWorld == null)
			return;

		Vec3 playerPos = mc.renderViewEntity.getPosition(0);
		List<Decal> decals = allDecals.get(dim);
		// Remove if expired, out of range, or parent block removed
		decals.removeIf(d -> d.shouldDie() || mc.theWorld.isAirBlock(d.blockX, d.blockY, d.blockZ) || Math.pow(d.x - playerPos.xCoord, 2) + Math.pow(d.y - playerPos.yCoord, 2) + Math.pow(d.z - playerPos.zCoord, 2) > 10000);
	}
}
