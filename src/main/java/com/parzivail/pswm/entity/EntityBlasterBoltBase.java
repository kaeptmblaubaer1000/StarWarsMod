...
public abstract class EntityBlasterBoltBase extends EntityThrowable
{
	...
	private void trackTarget()
	{
		if (this.target != null)
		{
			...
			double d1 = this.target.getEntityBoundingBox().minY + this.target.height / 3.0F - this.posY;
	...
	public EntityBlasterBoltBase(World par1World, EntityLivingBase par2EntityLivingBase, EntityLivingBase par3EntityLivingBase, float damage)
	{
		...
		double d1 = par3EntityLivingBase.getEntityBoundingBox().minY + par3EntityLivingBase.height / 3.0F - this.posY;
	...
	private void hitFX(int blockX, int blockY, int blockZ)
	{
		...
		//			StarWarsMod.network.sendToDimension(new MessageSpawnClientParticle("smoke", this.posX + (this.rand.nextFloat() - 0.5f) / 3, this.posY + (this.rand.nextFloat() - 0.5f) / 3, this.posZ + (this.rand.nextFloat() - 0.5f) / 3, motionX, motionY, motionZ), this.world.provider.dimensionId);
		//
		//			motionX = -this.motionX * 0.02f;
		//			motionY = this.rand.nextDouble() * 0.02f;
		//			motionZ = -this.motionZ * 0.02f;
		//			StarWarsMod.network.sendToDimension(new MessageSpawnClientParticle("blockdust_" + Block.getIdFromBlock(block) + "_" + this.world.getBlockMetadata(blockX, blockY, blockZ), this.posX + (this.rand.nextFloat() - 0.5f) / 3, this.posY + (this.rand.nextFloat() - 0.5f) / 3, this.posZ + (this.rand.nextFloat() - 0.5f) / 3, motionX, motionY, motionZ), this.world.provider.dimensionId);
		//		}
		//
	...
	{
		if (this.sender == null || this.world == null)
		{
			this.setDead();
			return;
		}

		if (pos.typeOfHit == RayTraceResult.Type.ENTITY && pos.entityHit != this.sender && pos.entityHit != this.sender.getRidingEntity())
		{
			if (pos.entityHit instanceof EntityPlayer)
			{
				...
				// TODO
				//				if (entityPlayer.isActiveItemStackBlocking() && entityPlayer.inventory.getCurrentItem() != null && entityPlayer.inventory.getCurrentItem().getItem() instanceof ItemLightsaber && ItemLightsaber.isOn(entityPlayer.inventory.getCurrentItem()))
				//				{
	...
	private void hit(RayTraceResult pos)
	{
		pos.entityHit.attackEntityFrom(PSWM.blasterDamageSource, this.damage);
		double f4 = MathHelper.sqrt(this.motionX * this.motionX + this.motionZ * this.motionZ);
	...
	{
		double f2 = MathHelper.sqrt(p_70186_1_ * p_70186_1_ + p_70186_3_ * p_70186_3_ + p_70186_5_ * p_70186_5_);
		p_70186_1_ /= f2;
		p_70186_3_ /= f2;
		p_70186_5_ /= f2;
		...
		double f3 = MathHelper.sqrt(p_70186_1_ * p_70186_1_ + p_70186_5_ * p_70186_5_);
