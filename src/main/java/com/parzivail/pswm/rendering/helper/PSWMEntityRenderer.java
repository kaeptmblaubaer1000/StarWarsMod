...
public class PSWMEntityRenderer extends EntityRenderer
{
	...
	public PSWMEntityRenderer(Minecraft p_i45076_1_, IResourceManager p_i45076_2_)
	{
	...
	public void renderWorld(float p_78471_1_, long p_78471_2_)
	{
		...
		for (int j = 0; j < 2; ++j)
		{
			...
			if (this.debugViewDirection == 0)
			{
				...
				net.minecraftforge.client.ForgeHooksClient.setRenderPass(0);
				// ToDo: Try and figure out how to make particles render sorted
				// correctly.. {They render behind water}
				RenderHelper.disableStandardItemLighting();
	...
	private void setupFog(int p_78468_1_, float p_78468_2_)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					double d0 = ((entitylivingbase.getBrightnessForRender(p_78468_2_) & 15728640) >> 20) / 16.0D + (entitylivingbase.lastTickPosY + (entitylivingbase.posY - entitylivingbase.lastTickPosY) * p_78468_2_ + 4.0D) / 32.0D;
			...
			if (this.mc.currentScreen != null)
			{
				...
				catch (Throwable throwable)
				{
					...
					crashreportcategory.addCrashSectionCallable("Screen name", new Callable()
					{
						...
						public String call()
						{
							return PSWMEntityRenderer.this.mc.currentScreen.getClass().getCanonicalName();
						}
					});
