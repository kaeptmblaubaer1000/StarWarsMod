...
public class GuiPowerList extends GuiScrollingList
{
	private static ResourceLocation background;
	private static ResourceLocation background2;
	private GuiScreenForcePowers parent;
	...
	public GuiPowerList(GuiScreenForcePowers parent, ArrayList<GuiPowerListItem> powers, int listWidth)
	{
	...
	public void drawScreen(int mouseX, int mouseY, float p_22243_3_)
	{
		...
		if (var19 > 0)
		{
			...
			Minecraft.getMinecraft().renderEngine.bindTexture(background2);
			tesselator.startDrawingQuads();
	...
	protected void drawSlot(int listIndex, int var2, int var3, int var4, Tessellator var5)
	{
		...
		boolean hasPower = power.level > 0;
		boolean isJedi = power.power.side == Force.ForceSide.JEDI;
		int c = isJedi ? (hasPower ? GLPalette.ANALOG_BLUE : GLPalette.BLUE) : (hasPower ? GLPalette.BRIGHT_RED : GLPalette.DARK_RED);
		c = power.power.side == null ? (hasPower ? GLPalette.BRIGHT_GREEN : GLPalette.MEDIUM_GREEN) : c;
		Minecraft.getMinecraft().fontRenderer.drawString(Minecraft.getMinecraft().fontRenderer.trimStringToWidth(power.power.translatedName, this.listWidth - 10), this.left + 3, var3 + 2, c);
		Minecraft.getMinecraft().fontRenderer.drawString(Minecraft.getMinecraft().fontRenderer.trimStringToWidth(LangUtils.translate("level.0", power.level), this.listWidth - 10), this.left + 3, var3 + 12, 0xCCCCCC);
	}
	...
	public static class GuiPowerListItem
	{
		@Nonnull
		final ForcePower power;
		final int level;

		public GuiPowerListItem(@Nonnull ForcePower power, int level)
		{
			this.power = power;
			this.level = level;
		}
